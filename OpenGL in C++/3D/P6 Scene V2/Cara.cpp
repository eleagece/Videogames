...
void Cara::dibuja(vector<PV3d*>* tv, vector<PV3d*>* tn)
 ...
 //---------------------------------------------------------------------------//
 glBegin(GL_POLYGON);
  int n=c[0]->getN();  // Sacamos de la cara el índice de la normal en el vector 'tn' para sacar la normal
  glNormal3d(tn->at(n)->getX(),tn->at(n)->getY(),tn->at(n)->getZ());  // Sacamos la normal del vector 'tn'
  for (int i=0; i<nelems; i++)
   {

...
HabitacionL::HabitacionL()//:ObjetoCompuesto3D()
 {
 // lampara (antes que todo para que se iluminen bien los objetos y no con retardo)
 lampara=new Lampara(1.5,1,0.6,10,10);
 lampara->setColor(0.8,0.8,0);
 lampara->getTAfin()->traslada(2.5,4.2,4);
 lampara->getTAfin()->rota(-90,1,0,0);
 objetos->push_back(lampara);
 // suelo
 Tablero* suelo=new Tablero(20,2,12,20,0.2,12,"baldosa.bmp",0,0,0,0,0,1,true);
